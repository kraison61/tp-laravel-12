    ...
    <div class="col-md-4">
        <div class="form-group">
            ...
            <div class="image-preview-box" onclick="document.getElementById('imgInput').click()"
                style="cursor: pointer; border: 2px dashed #ddd; border-radius: 8px; text-align: center; position: relative; min-height: 200px; display: flex; align-items: center; justify-content: center; overflow: hidden; background-color: #f9fafb;">

                <input type="file" id="imgInput" name="image" style="display:none" accept="image/*"
                    onchange="previewImage(this)">

                <div id="placeholder" class="upload-placeholder"
                    style="{{ isset($blog) && $blog->image ? 'display:none' : '' }}">
                    <i class="fa fa-cloud-upload" style="font-size: 40px; color: #aaa;"></i><br>
                    <span style="color: #666;">คลิกเพื่ออัปโหลดรูปภาพ</span>
                </div>

                <img id="preview" src="{{ isset($blog) && $blog->image ? Storage::disk('s3')->url($blog->image) : '' }}"
                    style="max-width: 100%; max-height: 100%; object-fit: cover; display: {{ isset($blog) && $blog->image ? 'block' : 'none' }};">
            </div>
        </div>
    </div>
</div>
    ...
    <script>
        $(document).ready(function () {
            $('#summernote').summernote({
                ...
                tabsize: 2,
                height: 400, // ปรับความสูงให้เยอะขึ้นหน่อยเพื่อความสบายตา
                dialogsInBody: true,
